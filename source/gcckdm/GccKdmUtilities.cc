	...
	if (node != NULL_TREE) {
		...
		case REFERENCE_TYPE: {
			...
			if (TREE_TYPE(node) == NULL) {
				...
				if (TYPE_NAME(node) && DECL_NAME(TYPE_NAME(node))) {
					...
					nameStr += " <T." + boost::lexical_cast<std::string>(
							TYPE_UID(node)) + ">";
				}
				nameStr += ")";
		...
		case TYPE_DECL: {
			...
			if (DECL_NAME(node)) {
				...
				nameStr = TREE_CODE(TREE_TYPE(node)) == UNION_TYPE ? "union"
						: "struct ";
		...
		default: {
			std::cerr << "gcckdm::getAstNodeName() not implemented yet: "
					<< tree_code_name[TREE_CODE(node)] << std::endl;
		}
